...
from pyrogram.enums import ChatType, ChatMemberStatus, ParGoldde, ChatMemberStatus
import os
import re
import textwrap
import aiofiles
...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "F88EE":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "فولت")
      except:
        pass
...
async def F88EE(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝗦𝗢𝗨𝗥𝗖𝗘 𝗔𝗟𝗤𝗔𝗜𝗗](t.me/qqvqqs) 🔰 .
么 [𝐌𝐎𝐑𝐀𝐄𝐁](t.me/F88EE) 👑 .
么 [𝐆𝐑𝐎𝐔𝐏](t.me/lIIIllIllll) ™️  . 
╰──── • ◈ • ────╯
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton("√ َِ𝐌𝐎𝐑𝐀𝐄𝐁™|| » مـرعـب ", user_id=f"F88EE")
                ],
                [
                    InlineKeyboardButton("اضف البوت الي مجموعتك ❤️", url="https://t.me/{bot.username}?startgroup=true")
                ],
...
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="F88EE")
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["F88EE"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["F88EE"]:
   ...
   if message.from_user.username == "F88EE":
     await message.reply_text(f"**تم تنفيذ الامر بواسطة المطور فولت  🚦 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**➧ 𝙅𝙊𝙄𝙉 |⌯ @qqvqqs  🚦 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**➧ 𝙅𝙊𝙄𝙉 |⌯ @qqvqqs  🚦 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**➧ 𝙅𝙊𝙄𝙉 |⌯ @qqvqqs  🚦 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["F88EE"]:
         await message.reply_text("**رتبتك هي : المطور فولت  🚦 .**")
         return
